...
async def canvas_login():
    """
    Redirects the user to Canvas OAuth login page
    """
    oauth_url = (
...
async def canvas_callback(request: Request, code: str):
    """
    Canvas redirects here with an authorization code.
    ...
    async with httpx.AsyncClient() as client:
        user_resp = await client.get(f"{CANVAS_BASE}/api/v1/users/self", headers={
